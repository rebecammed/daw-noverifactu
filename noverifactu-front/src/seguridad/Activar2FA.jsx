...
function Activar2FA() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    authFetch("/api/2fa/status") // Necesitarás este endpoint en el backend
      .then((res) => res.json())
      .then((data) => {
        if (data.enabled) {
          setActivado(true);
          setMensaje("La autenticación 2FA ya está activa en tu cuenta.");
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  async function generarQR() {
    ...
    try {
      const res = await authFetch("/api/2fa/setup", {
        method: "POST",
      });
  ...
  async function verificarCodigo(e) {
    ...
    try {
      ...
      setMensaje("✅ Autenticación en dos factores activada");
      setActivado(true);
    } catch (e) {
      console.error(e);
  ...
  if (loading) return <p>Cargando estado de seguridad...</p>;

  return (
    <div>
      <h2>Autenticación en dos factores (2FA)</h2>

      {!qr && !activado && <button onClick={generarQR}>Activar 2FA</button>}
